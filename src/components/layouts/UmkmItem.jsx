...
export default function UmkmItem({ image, name, slug, price, fullWidth }) {
  return (
    <div
      ...
      } flex-shrink-0 border border-gray-200 shadow-lg rounded-lg hover:shadow-2xl transition duration-300 md:min-w-[25%]`}
    >
      <Image
        priority
        src={image}
      ...
      <div className="p-4">
        ...
        <Link href={`/umkm/${slug}`}>
          <p className="mt-3 text-white text-sm bg-sky-500 py-2 px-4 rounded hover:bg-sky-400 transition duration-300 w-max ms-auto flex justify-center items-center gap-1.5">
            Lihat <IoEyeOutline className="text-lg mt-0.5" />
          </p>
        </Link>
      </div>
    </div>
